...
begin
  ...
  ComponentValues := 'Компоненти які будуть помічені NP' + #13#10;
        ...
        begin
          ...
          begin
            ...
            OriginalValue := Parameter.Text;
              ...
              begin
                // заміна uF на мкФ в полі "Value"
                OriginalValue := Parameter.Text;
                Parameter.Text := ReplaceStr(OriginalValue, 'uF', 'мкФ');
              end;
              if Pos('uH', OriginalValue) > 0 then
              begin
                // заміна uH на мкГн в полі "Value"
                OriginalValue := Parameter.Text;
                Parameter.Text := ReplaceStr(OriginalValue, 'uH', 'мкГн');
              end;
              if Pos('кОм', OriginalValue) > 0 then
              begin
                // заміна кОм на к в полі "Value"
                Parameter.Text := ReplaceStr(OriginalValue, 'кОм', 'к');
              end;
